...
ON('load-record', function(event) {
  var adminRoles = ['Owner', 'Maestro User'];
  ...
  if (ISROLE(adminRoles)) {
    ...
    if (ISROLE(['Owner', 'Call Center Representative', 'Maestro User'])) {
...
ON('click', 'all_meters_for_this_service', function (event) {
  var options = {
    ...
    qs: {
      token: 'f0c94ed3ffee26270fc292a5b4a7d89f9bf6f27c4de64b6f50a115d958bcbbc8',
      format: 'json',
      q: 'SELECT _record_id FROM "NYC AMR Meters 2019" WHERE address = \'' + $address + '\' AND city = \'' + $city + '\''
    }
  };
  ...
  if (ISROLE('National Grid CMS Field User') && ISMOBILE()  ) {
    SETHIDDEN('inspection_completed_date', true);
    SETHIDDEN('poe_appointment', true);
        }
