    ...
    SETVALUE('inspection_completed_date',TIMESTAMP());
      ...
      var options = {
        ...
        qs: {
          ...
          format: 'json',
          q: 'SELECT count(_record_id) as count_ids FROM "NG NYC Meters" WHERE inspections is not null and address = \'' + $address + '\''
        }
     };
...
ON('click', 'all_meters_for_this_service', function (event) {
  var options = {
    ...
    qs: {
      token: 'f0c94ed3ffee26270fc292a5b4a7d89f9bf6f27c4de64b6f50a115d958bcbbc8',
      format: 'json',
      q: 'SELECT _record_id FROM "NG NYC Meters" WHERE address = \'' + $address + '\' AND city = \'' + $city + '\''
    }
  };
