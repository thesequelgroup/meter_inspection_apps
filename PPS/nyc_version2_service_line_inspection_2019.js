...
ON('load-record', function(event) {
  ...
  if (ISROLE(fieldUserRoles)) {
    ...
    SETSTATUSFILTER(['Door Hanger','Revisit 1st Attempt DH', 'Revisit 2nd Attempt DH', 'Missed Appointment', 'Cement Sealing Appt Required', 'Complete', 'Customer Refused', 'Unable To Post', 'School']);
  }
  ...
  if (ISROLE(fieldUserRoles)) {
    // set the status field filter
    SETSTATUSFILTER(['Post Card First Attempt', 'BD First Attempt', 'Unassigned Appointment', '8AM-Noon', 'Noon-4PM', '4PM-7PM', 'Door Hanger', 'Revisit 1st Attempt DH', 'Revisit 2nd Attempt DH', 'Missed Appointment', 'Cement Sealing Appt Required', 'Complete' ,'Customer Refused', 'Unable To Post', 'School', 'Previously Inspected']);
  }
...
ON('change-status', function (event) {
  ...
  if($is_meter_outside=='yes' && event.value != 'Complete' && event.value != 'Cement Sealing Appt Required')
  {
    incorrect=1;
  ...
  if((event.value == 'Complete' || event.value == 'Cement Sealing Appt Required') && ($is_meter_outside!='yes' && EXISTS(REPEATABLEVALUES($meters, 'location_of_meter'))))
  {
      var locations = [];
...
ON('validate-record', function (event) {
 if($is_meter_outside=='yes' && STATUS() != 'Complete' && event.value != 'Cement Sealing Appt Required')
  {
