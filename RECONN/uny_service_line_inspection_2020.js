...
ON('load-record', function(event) {
  ...
  if (ISROLE(fieldUserRoles)) {
    ...
    SETSTATUSFILTER(['Missed Appointment', 'Door Hanger', 'Complete', 'Opportunistic-Reconn', 'Customer Refused', 'In Progress', 'Unable to Post', 'POE Appointment']);
  }
  ...
  if (ISROLE(adminUserRoles)) {
    // set the status field filter
    SETSTATUSFILTER(['Missed Appointment', 'Door Hanger', 'Complete', 'Unassigned Appointment', '8AM-Noon', 'Noon-4PM', 'In Progress', 'Customer Refused', 'Unable to Post', '4PM-7PM', 'POE Appointment']);
  }
...
ON('change-status', function (event) {
  ...
  if(event.value == 'Complete' && $is_meter_outside!='yes' && $requirements!=1)
  {
      ...
      if(EXISTS(REPEATABLEVALUES($meter_inspection_parent, 'location_of_meter')))
        locations = REPEATABLEVALUES($meter_inspection_parent, 'location_of_meter').map(CHOICEVALUE);
    ...
    if(EXISTS(gas_leak) && gas_leak[0]=='yes')
     {
        ...
        SETREQUIRED('location_security_information',false);
    ...
    if(
          (
            event.value == 'POE Appointment' || event.value == 'Renewed in Fulcrum' || event.value == 'UNY Opportunistic' || event.value ==  'Complete') && incorrect==0
...
ON('validate-record', function (event) {
 if($is_meter_outside=='yes' && STATUS() != 'Complete')
  ...
  if(STATUS() == 'Complete' && $is_meter_outside!='yes' && $requirements!=1)
  {
     ...
     if($meter_inspection_parent!= null && $meter_inspection_parent)
      {
          for (var i = 0; i < locations.length; i++) 
          {
     ...
     if(EXISTS(gas_leak) && gas_leak[0]=='yes' && STATUS() == 'Complete')
     {
        SETREQUIRED('is_meter_outside',false);
        SETREQUIRED('location_security_information',false);
        SETREQUIRED('poe_inspection_parent',false);
    ...
    if(EXISTS(REPEATABLEVALUES($poe_inspection_parent,'poe_inspection_status')))
...
ON('click', 'call_dispatch_immediately', function(event) {
  if (!EXISTS('cms_dispatch')) {
    ALERT('You Must Enter A Phone Number.');
    return;
  }
